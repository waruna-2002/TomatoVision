...
def detect_ultimate_agro_isolated(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # =========================================================================
    # STEP 1: ISOLATE THE TOMATO HEAP / CRATE AREA (තක්කාලි ඇති Area එක වෙන් කර ගැනීම)
    # =========================================================================
    # Strict Carotenoid Anchor: S > 130 guarantees zero overlap with green chilies, radishes, potatoes, sand
    red_orange_core = (
    ...
    if contours:
        for c in contours:
            ...
            # Ignore edge strips
            if area > 2500 and not (bw > 0.80 * w and by <= 5 and bh < 0.12 * h):
                valid_c.append(c)
...
images = {
    "1. Market Stall (Chilies & Radishes)": img1,
    "2. Market Stall (Lettuce, Cabbage, Potatoes, Sand)": img2,
    "3. Crate of Tomatoes": img3,
    "4. Single Green Tomato on Paper": img4
}

for name, img in images.items():
    res = detect_ultimate_agro_isolated(img)
    counts = {}
    for r in res:
        c = r["class_name"]
