...
def detect_tomatoes_perfect_agro(img_bgr):
    ...
    # Red & Orange Seeds (Strictly inside real tomatoes, 0% overlap with chilies, radishes, potatoes, sand)
    red_seeds = (
        ((h_chan <= 13) | (h_chan >= 165)) & (s_chan > 115) & (v_chan > 65) |
        ((h_chan > 13) & (h_chan <= 24)) & (s_chan > 130) & (v_chan > 70)
    ).astype(np.uint8) * 255

    # Filter out table strips from red_seeds (remove horizontal bands touching top/bottom border)
    ...
    if c_list:
        for c in c_list:
            ...
            # Reject horizontal table strips / wall boundaries (AR > 2.5 or glued to top/bottom border)
            if ar > 2.5 or ar < 0.40 or (cy <= 5 and cw > 0.60 * w):
                continue
            valid_clusters.append((c, cx, cy, cw, ch, c_area))

    # =========================================================================
    # SCENARIO A: DENSE TOMATO HEAP / CRATE ISOLATED
    # (When a legitimate Red/Orange Tomato Pile exists in the scene)
    # =========================================================================
    if valid_clusters:
    ...
    # =========================================================================
        ...
        for c in contours:
            ...
            if 0.45 <= ar <= 2.3 and circ >= 0.25:
                if not (cw > 0.65 * w and (y <= 5 or ch < 0.20 * h)):
...
images = {
    ...
    "2. Crate": (cv2.imread(r"C:\Users\Waruna\.gemini\antigravity\brain\d72fea25-187d-4266-99c6-341a7dceced8\.user_uploaded\media_1787102591917.png")[58:518, 412:612]),
    "3. Market 1 (Potatoes & Sand)": (cv2.imread(r"C:\Users\Waruna\.gemini\antigravity\brain\d72fea25-187d-4266-99c6-341a7dceced8\.user_uploaded\media_1787102634519.png")[58:518, 412:612]),
    "4. Market 2 (Radishes & Chilies)": (cv2.imread(r"C:\Users\Waruna\.gemini\antigravity\brain\d72fea25-187d-4266-99c6-341a7dceced8\.user_uploaded\media_1787102613151.png")[58:518, 412:612]),
}
