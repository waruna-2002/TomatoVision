...
def detect_tomatoes(img_bgr):
    ...
    # Red & Orange Seeds (Strictly anchor Tomato Heaps, 0% overlap with green chilies, cucumbers, beetroots, sand)
    red_seeds = (
    ...
    # A legitimate tomato pile is compact (AR 0.45 - 2.2, Area > 3500px, not a thin frame border strip)
    valid_clusters = []
    if c_list:
        for c in c_list:
    ...
    # -------------------------------------------------------------------------
    # BRANCH 1: TOMATO HEAP / CRATE (When a Valid Compact Tomato Cluster Exists)
    # -------------------------------------------------------------------------
    if valid_clusters:
    ...
    # -------------------------------------------------------------------------
        ...
        mask_tomato = (
            ((h_chan <= 15) | (h_chan >= 162)) & (s_chan > 60) & (v_chan > 45) |
            ((h_chan > 15) & (h_chan <= 28)) & (s_chan > 70) & (v_chan > 50) |
            ((h_chan > 28) & (h_chan <= 38)) & (s_chan > 70) & (v_chan > 50) |
            ((h_chan > 38) & (h_chan <= 58)) & (s_chan > 60) & (v_chan > 45)
        ).astype(np.uint8) * 255

        mask_tomato[h_chan > 58] = 0
        mask_tomato[(h_chan >= 120) & (h_chan <= 165)] = 0
        mask_tomato[s_chan <= 45] = 0
        ...
        return "SINGLE/STANDALONE MODE", segmented
...
mode1, res1 = detect_tomatoes(img_g)
...
mode2, res2 = detect_tomatoes(img_m)
...
mode3, res3 = detect_tomatoes(img_c)
