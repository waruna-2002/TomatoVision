...
def detect_tomatoes_two_stage_final(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # =========================================================================
    tomato_seeds = (
        ((h_chan <= 13) | (h_chan >= 165)) & (s_chan > 120) & (v_chan > 60) |
        ((h_chan > 13) & (h_chan <= 25)) & (s_chan > 130) & (v_chan > 65)
    ).astype(np.uint8) * 255
    ...
    if heap_contours:
        for c in heap_contours:
            ...
            # A real tomato heap is a substantial cluster (Area > 8000 or > 2.5% of frame)
            if c_area > max(8000, 0.025 * img_area):
                hx, hy, hw, hh = cv2.boundingRect(c)
                ar = float(hw) / hh if hh > 0 else 0
                # Filter out straight table strips (AR > 2.8 or thin strip at top edge)
...
for name, img in raw_images.items():
    mode, res = detect_tomatoes_two_stage_final(img)
    counts = {}
    for r in res:
        c = r["class_name"]
