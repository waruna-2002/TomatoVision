...
def detect_tomatoes_flawless(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # -------------------------------------------------------------------------
    # 1. PURE TOMATO PIGMENT ANCHOR (S > 125 eliminates all dirt/sacks/floors/vegetables)
    # -------------------------------------------------------------------------
    tomato_seeds = (
        ((h_chan <= 13) | (h_chan >= 165)) & (s_chan > 125) & (v_chan > 60) |
        ((h_chan > 13) & (h_chan <= 25)) & (s_chan > 135) & (v_chan > 65)
    ).astype(np.uint8) * 255
    ...
    if heap_contours:
        for c in heap_contours:
            ...
            # Must be a dense cluster (Area > 8000 on full images or > 2.5% of frame)
            if c_area > max(8000, 0.025 * img_area):
                hx, hy, hw, hh = cv2.boundingRect(c)
                ar = float(hw) / hh if hh > 0 else 0
                # Filter out straight table strips (AR > 3.0 or thin strip at top edge)
                if ar <= 2.8 and not (hw > 0.70 * w and (hy <= 5 or hh < 0.12 * h)):
    ...
    if valid_heaps:
        ...
        # Strict ROI: Tomato Heap area only!
        ...
        roi_orange = (roi_h > 14) & (roi_h <= 26) & (roi_s > 80) & (roi_v > 50)
        roi_yellow = (roi_h > 26) & (roi_h <= 36) & (roi_s > 80) & (roi_v > 50)
    ...
    # =========================================================================
        ...
        mask_tomato = (
            ((h_chan <= 14) | (h_chan >= 162)) & (s_chan > 40) & (v_chan > 45) |
