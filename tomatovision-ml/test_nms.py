...
local_max = (dist == dilated) & (dist > 4.5) & (dist > 0.10 * dist.max())
...
# Sort by distance transform peak value descending
raw_centers.sort(key=lambda x: x[2], reverse=True)

# Min center-to-center distance suppression
suppressed = []
min_dist = 16.0 # Min pixels between tomato centers in crate

for c in raw_centers:
    cx, cy, d_val = c
    too_close = False
    for s in suppressed:
        dist_sq = (cx - s[0])**2 + (cy - s[1])**2
...
print(f"Total True Tomatoes Detected in Crate: {len(tomatoes)}")
print(f"4 Category Counts: {counts}")
cv2.imwrite("d:/project/TomatoVision/tomatovision-ml/nms_detected.jpg", output_img)
