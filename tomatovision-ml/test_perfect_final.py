...
def detect_tomatoes_perfect_final(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # 1. PURE TOMATO PIGMENT SEEDS (S > 110 eliminates sand, dirt sacks, floors, chilies, radishes, potatoes)
    tomato_seeds = (
        ((h_chan <= 13) | (h_chan >= 165)) & (s_chan > 110) & (v_chan > 55) |
        ((h_chan > 13) & (h_chan <= 25)) & (s_chan > 120) & (v_chan > 60)
    ).astype(np.uint8) * 255
    ...
    if heap_contours:
        for c in heap_contours:
            ...
            if c_area > max(5000, 0.02 * img_area):
                hx, hy, hw, hh = cv2.boundingRect(c)
                ar = float(hw) / hh if hh > 0 else 0
                # ONLY reject thin horizontal table strips (AR > 3.0 or thin strip < 12% frame height at top edge)
                is_thin_table_strip = (ar > 3.0) or (hw > 0.70 * w and hy <= 5 and hh < 0.12 * h)
                if not is_thin_table_strip:
                    valid_heaps.append((c, hx, hy, hw, hh, c_area))

    # =========================================================================
    # BRANCH 1: DENSE TOMATO HEAP / CRATE (තක්කාලි ගොඩක්)
    # =========================================================================
    if valid_heaps:
        all_heap_detections = []
        for heap in valid_heaps:
            _, hx, hy, hw, hh, _ = heap
            ...
            roi_red2 = (roi_h >= 162) & (roi_s > 60) & (roi_v > 45)
            ...
            kernel_peak = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (17, 17))
            dilated = cv2.dilate(dist, kernel_peak)
            local_max = (dist == dilated) & (dist > 3.5) & (dist > 0.06 * dist.max())
            ...
            for cx, cy, d_val in suppressed:
                ...
                all_heap_detections.append({
                    "class_name": stage,
                    "confidence": conf,
                    "box": [float(gx1), float(gy1), float(gx2), float(gy2)]
                })

        return "HEAP MODE", all_heap_detections
    ...
    # =========================================================================
        ...
        for c in contours:
            ...
            if 0.45 <= ar <= 2.4 and circ >= 0.25:
                if not (cw > 0.65 * w and hy <= 5 and ch < 0.18 * h):
...
raw_images = {
    "1. Market Stall Photo": cv2.imread(r"C:\Users\Waruna\.gemini\antigravity\brain\d72fea25-187d-4266-99c6-341a7dceced8\.user_uploaded\media_1787075584775.jpg"),
...
for name, img in raw_images.items():
    mode, res = detect_tomatoes_perfect_final(img)
    counts = {}
    for r in res:
        c = r["class_name"]
