...
def detect_tomatoes_perfect_production(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # 1. PURE TOMATO PIGMENT SEEDS (S > 115 eliminates sand, dirt sacks, floors, chilies, radishes, potatoes)
    tomato_seeds = (
    ...
    if heap_contours:
        for c in heap_contours:
            ...
            if c_area > max(5000, 0.02 * img_area):
                ...
                # Exclude horizontal table strips (AR > 3.0 or thin strip at top edge)
...
for name, img in raw_images.items():
    mode, res = detect_tomatoes_perfect_production(img)
    counts = {}
    for r in res:
        c = r["class_name"]
