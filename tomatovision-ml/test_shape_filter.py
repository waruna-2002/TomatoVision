...
def detect_tomatoes_by_area_and_shape(img_bgr):
    h, w = img_bgr.shape[:2]
    img_area = float(w * h)
    ...
    # =========================================================================
    # STEP 1: ISOLATE THE PRIMARY TOMATO AREA (තක්කාලි ඇති ප්‍රධාන Area එක මුලින්ම හඳුනා ගැනීම)
    # =========================================================================
    # High-saturation Red & Orange tomato pigment anchor (Strictly tomatoes, S > 115 eliminates sand, dirt, cloth, potatoes)
    tomato_seeds = (
    ...
    if heap_contours:
        for c in heap_contours:
            ...
            if c_area > max(15000, 0.05 * img_area):
                hx, hy, hw, hh = cv2.boundingRect(c)
                ar = float(hw) / hh if hh > 0 else 0
                # Exclude horizontal table strips
                if 0.30 <= ar <= 2.8 and not (hw > 0.70 * w and hy <= 5 and hh < 0.12 * h):
    ...
    # =========================================================================
    if valid_heaps:
        # Pick the TRUE PRIMARY TOMATO HEAP (The largest cluster in the frame)
        primary_heap = max(valid_heaps, key=lambda x: x[5])
        _, hx, hy, hw, hh, _ = primary_heap

        # 1. STRICT TOMATO HEAP ROI (100% isolates the Tomato Area, ignoring everything else!)
        ...
        roi_red1 = (roi_h <= 14) & (roi_s > 65) & (roi_v > 45)
        roi_red2 = (roi_h >= 162) & (roi_s > 65) & (roi_v > 45)
        roi_orange = (roi_h > 14) & (roi_h <= 26) & (roi_s > 75) & (roi_v > 50)
        roi_yellow = (roi_h > 26) & (roi_h <= 36) & (roi_s > 75) & (roi_v > 50)
        roi_unripe = (roi_h > 36) & (roi_h <= 60) & (roi_s > 55) & (roi_v > 50)
        ...
        if dist.max() > 0:
            ...
            for cx, cy, d_val in suppressed:
                ...
                # STEP 2: CLASSIFICATION BY RIPENESS
                if dark_spots > 0.40:
                    stage = "spoiled"
                    conf = 0.92
    ...
    # =========================================================================
        ...
        # STEP 2: DETECT BY COLOR + ROUND SHAPE (Circularity >= 0.45, Aspect Ratio 0.5 - 2.0)
        mask_tomato = (
        ...
        for c in contours:
            c_area = cv2.contourArea(c)
            # Must be a substantial fruit, not tiny noise
            ...
            # Reject long thin objects (chilies), rectangular strips (table), or jagged non-fruit noise
        ...
        # Single fruit mode should have at most 2 fruits
        if len(detections) > 2:
...
raw_images = {
    "1. Market Stall with Chilies & Radishes": cv2.imread(r"C:\Users\Waruna\.gemini\antigravity\brain\d72fea25-187d-4266-99c6-341a7dceced8\.user_uploaded\media_1787075584775.jpg"),
...
for name, img in raw_images.items():
    mode, res = detect_tomatoes_by_area_and_shape(img)
    counts = {}
    for r in res:
        c = r["class_name"]
